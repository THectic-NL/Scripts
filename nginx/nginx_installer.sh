...
case "$COMMAND" in
    install)
        Test-MutableOs "NGINX" \
            "Run it as a container: podman run -d -p 80:80 docker.io/library/nginx" \
            "distrobox: create a mutable container and build NGINX inside it" \
            "Layer it onto the image: rpm-ostree install nginx"

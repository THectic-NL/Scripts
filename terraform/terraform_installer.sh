...
Test-MutableOs "Terraform" \
    "distrobox: create a mutable container and install Terraform inside it" \
    "Homebrew: brew install terraform" \
    "Download the official binary from https://releases.hashicorp.com/terraform/" \
    "Layer it onto the image: rpm-ostree install terraform"
